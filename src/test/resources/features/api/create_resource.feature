...
Feature: Create a new book
    ...
    Scenario: Successfully create a new book as an admin
      ...
      When Admin creates a new book with title <title> and author <author>
    ...
    Scenario: User attempts to create a book without an author
            Given User is authorized as a regular user
            When User creates a new book with a title <title> and without an author
            Then the application should return status code 400 for bad request
            And the response message should be "Author is a mandatory field"
            Examples:
              | title       |

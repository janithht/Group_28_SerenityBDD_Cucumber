...
Feature: Book update
  ...
  Scenario: Successfully update a book details
    ...
    When Admin updates the book with ID <bookId> setting title to <title>, author to <author>
  ...
  Scenario: Update with Authentication Errors
    Given User is authorized as an admin
    And a book exists with Id 1
    But the Admin sends a PUT request to update the book with ID 1 without proper authentication credentials
